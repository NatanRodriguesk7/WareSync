...
function renderDashboard(resumo) {
  ...
  const vendasRecentesRows = resumo.vendas_recentes.length
    ? resumo.vendas_recentes
        ...
        <tr>
          ...
          <td class="actions"><button class="btn-secondary" data-devolver="${v.id}">Devolver</button></td>
        </tr>`
        )
        .join('')
  ...
  document.querySelectorAll('[data-devolver]').forEach((btn) => {
    btn.addEventListener('click', () => abrirDevolucao(Number(btn.dataset.devolver)));
  });
}

async function abrirDevolucao(vendaId) {
  const res = await fetch(`${API_BASE}/vendas/${vendaId}`);
  const { data: venda } = await res.json();

  if (!venda.itens || venda.itens.length === 0) {
    alert('Esta venda não possui itens.');
    return;
  }

  const linhas = venda.itens
    .map((item, idx) => `${idx + 1}. ${item.produto_nome} — vendido: ${item.quantidade}`)
    .join('\n');

  const escolha = prompt(`Itens da venda #${venda.id}:\n${linhas}\n\nDigite o número do item a devolver:`);
  if (!escolha) return;

  const item = venda.itens[Number(escolha) - 1];
  if (!item) {
    alert('Item inválido.');
    return;
  }
  ...
  const quantidade = Number(
    prompt(`Quantidade a devolver de "${item.produto_nome}" (máx ${item.quantidade}):`, item.quantidade)
  );
  if (!quantidade || quantidade <= 0) return;

  const motivo = prompt('Motivo da devolução (opcional):') || '';

  const devRes = await fetch(`${API_BASE}/devolucoes`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ venda_id: venda.id, motivo, itens: [{ venda_item_id: item.id, quantidade }] }),
  });

  const devData = await devRes.json();

  if (!devData.success) {
    alert(devData.message || 'Não foi possível registrar a devolução.');
    return;
  }

  alert('Devolução registrada e estoque reposto.');
  carregarDashboard();
}
